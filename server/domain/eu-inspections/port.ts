...
// keyed by id, like employees/vehicles — (vehicleId, dueDate) is a unique
// constraint used as ensure()'s upsert target, not the PK.
export interface EuInspectionPort
  extends
    ByKey<EuInspection, string>,
    Pageable<EuInspection>,
    Countable,
    Updatable<EuInspection, string> {
  /** Upsert an inspection by (vehicleId, dueDate), assigning `id` on insert.
