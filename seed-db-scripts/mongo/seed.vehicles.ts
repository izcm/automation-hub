import { vehicles, employees } from "../../server/db/mongo/collections";
import type { VehicleDoc } from "../../server/db/mongo/types/vehicle-doc";
...
async function seed() {
  ...
  const employeeDocs = await employees().find().toArray();
  if (employeeDocs.length === 0) {
    throw new Error("No employees found — run `npm run seed:employees` first.");
  }
  const employeeIds = employeeDocs.map((e) => e.id);
  ...
  console.log(`✅ seeded ${res.insertedCount} vehicles (linked to employees)`);
  process.exit(0);
}
